...
void IDFI2CBus::setup() {
  ...
  printf("I2C Bus setup completed\n");
  ESP_LOGI(TAG, "I2C Bus setup completed");
}

void IDFI2CBus::dump_config() {
//   ESP_LOGCONFIG(TAG, "I2C Bus:");
printf("  I2C Bus:\n");
    printf("  SDA Pin: GPIO%u\n", this->sda_pin_);
    printf("  SCL Pin: GPIO%u\n", this->scl_pin_);
...
ErrorCode IDFI2CBus::write_readv(uint8_t address, const uint8_t *write_buffer, size_t write_count, uint8_t *read_buffer,
  ...
  if (read_count == 0 && write_count == 0) {
    ...
    if (write_count != 0) {
      ESP_LOGV(TAG, "0x%02X TX %s", address, format_hex_pretty(write_buffer, write_count, ' ', false ).c_str());
      jobs[num_jobs++].command = I2C_MASTER_CMD_START;
      jobs[num_jobs].command = I2C_MASTER_CMD_WRITE;
      jobs[num_jobs].write.ack_check = true;
      jobs[num_jobs].write.data = &write_addr;
  ...
  if (err == ESP_ERR_INVALID_STATE) {
    ...
    if(write_count > 0) {
      ESP_LOGV(TAG, "Wrote to REG %02X, Val:%s", write_addr, format_hex_pretty(write_buffer, write_count, ' ', false ).c_str());
    }
    if (read_count > 0) {
      ESP_LOGV(TAG, "Read from REG:%02X, Val: %s", read_addr, format_hex_pretty(read_buffer, read_count, ' ', false ).c_str());
    }
  }
  return ERROR_OK;
}
...
void IDFI2CBus::recover_() {
  ...
  esp_rom_delay_us(half_period_usec);
  ...
  for (auto i = 0; i < 9; i++) {
    ...
    while (wait-- && gpio_get_level(scl_pin) == 0) {
      ...
      esp_rom_delay_us(half_period_usec * 2);
    }
    if (gpio_get_level(scl_pin) == 0) {
  ...
  printf("I2C bus recovery completed successfully.\n");
}
