...
static void anim_timer(lv_timer_t * param)
{
    ...
    while(a != NULL) {
        ...
        //printf("objcnt: %d; a->act_time: %d\n", objcnt, (int)a->act_time);
        objcnt++;
        ...
        if(!a->is_paused && a->run_round != state.anim_run_round) {
            ...
            if(a->act_time >= 0) {
                ...
                if(new_value != a->current_value) {
                    ...
                    /*Apply the calculated value actual function call is 'scroll_y_anim(a->var, new_value)'; found in 'lv_obj_scroll.c' */
                    //printf("a->exec_cb(a->var, new_value) START\n");
                    if(a->exec_cb) a->exec_cb(a->var, new_value);
                    //printf("a->exec_cb(a->var, new_value) COMPETE\n\n");
                ...
                //else printf("state.anim_list_changed!!\n");
            }
        }
