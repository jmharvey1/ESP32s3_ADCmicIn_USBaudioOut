...
//jmh add
#define USB_DEVICE_UAC_VER_MAJOR 0
#define USB_DEVICE_UAC_VER_MINOR 0
#define USB_DEVICE_UAC_VER_PATCH 0
...
void tud_audio_feedback_params_cb(uint8_t func_id, uint8_t alt_itf, audio_feedback_params_t* feedback_param)
{
    ...
    // sprintf(TxtBuf,"%s: Feedback method: %d, sample freq: %"PRIu32"\n", TAG, feedback_param->method, feedback_param->sample_freq);
...
static void usb_mic_task(void *pvParam)
{
    while (1)
    {
        ...
        {
            // clear the notification
            /*JMH This only happens once per active input session*/
            ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
            continue;
        }
        // clear the notification
        ...
        size_t bytes_require = MIC_INTERVAL_MS * s_uac_device->mic_bytes_per_ms;
        if (s_uac_device->user_cfg.input_cb) // verify input callback exists
        {
            size_t bytes_read = 0;
