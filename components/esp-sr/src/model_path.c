...
srmodel_list_t *srmodel_load(const void *root)
{
    ...
    for (int i = 0; i < models->num; i++) {
        ...
        /*JMH Added the following two lines*/
        printf("srmodel_load(); file_num:%d\n",file_num);
        if(file_num > 50) return NULL;
...
srmodel_list_t *esp_srmodel_init(const char *partition_label)
{
    ...
    if (partition)
    {
        sprintf(TxtBuf, "FOUND %s in partition table\n", partition_label);
    ...
    else
    {
        ...
        sprintf(TxtBuf, "Can not find %s in partition table\n", partition_label);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
