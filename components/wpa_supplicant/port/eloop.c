...
int eloop_init(void)
{
    ...
    if (!eloop_data_lock) {
        ...
        printf("failed to create eloop data loop\n");
        return -1;
    }
...
overflow:
    ...
    //           secs, usecs);
    printf("Too long timeout (secs=%u usecs=%u) to ever happen - ignore it\n",
           secs, usecs);
    os_free(timeout);
    return 0;
}
...
#endif
{
    ...
    if (!eloop.eloop_semph) {
        ...
        printf("ELOOP: sync semphr not available\n");
        os_free(timeout);
        return -1;
    }
...
run:
    ...
    printf("ELOOP: waiting for sync semphr\n");
