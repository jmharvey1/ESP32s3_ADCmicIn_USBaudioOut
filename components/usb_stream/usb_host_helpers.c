...
//#include "usbd.h"//JMH Added
#include "tusb_config.h"//JMH Added
#include "tinyusb.h"//JMH Added
...
{
    const char *MyTAG = {"\t_usb_port_init()"};
    char TxtBuf[50];
    if ((callback == ((void *)0) || callback_arg == ((void *)0)))
    {
        sprintf(TxtBuf, "%s:\tcallback &/or cb_arg MISSING\n", MyTAG);
    ...
    usb_phy_config_t phy_config = {
        ...
        .otg_mode = USB_OTG_MODE_DEVICE, // USB_OTG_MODE_HOST,
        .otg_speed = USB_PHY_SPEED_FULL, // USB_PHY_SPEED_UNDEFINED,   //In Host mode, the speed is determined by the connected device
    ...
    if (ret != ESP_OK)
    {
        sprintf(TxtBuf, "%s:\tUSB PHY init failed\n", MyTAG);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    //     sprintf(TxtBuf, "USB Device Stack Init FAILED!!\n");
    //     if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    //     {
    //         Print2Dsply(TxtBuf);
    //         xSemaphoreGive(Txt2Dsply_mutx);
    ...
    // if (ret_val != pdPASS)
    // {
    //     sprintf(TxtBuf, "Failed to create TinyUSB task!!\n");
    //     if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    //     {
    //         Print2Dsply(TxtBuf);
    //         xSemaphoreGive(Txt2Dsply_mutx);
    ...
    // else
    // {
    //     sprintf(TxtBuf, "TinyUSB task Running!!\n");
    //     if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    //     {
    //         Print2Dsply(TxtBuf);
    //         xSemaphoreGive(Txt2Dsply_mutx);
    ...
    if (ret != ESP_OK)
    {
        sprintf(TxtBuf, "%s:\tHCD Install failed\n", MyTAG);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    if (ret != ESP_OK)
    {
        sprintf(TxtBuf, "%s:\tHCD Port init failed\n", MyTAG);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    sprintf(TxtBuf, "%s:\tPort=%d init succeed, ", MyTAG, USB_PORT_NUM);
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
        xSemaphoreGive(Txt2Dsply_mutx);
    }
    sprintf(TxtBuf, "context = %p\n", phy_handle);
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
        xSemaphoreGive(Txt2Dsply_mutx);
