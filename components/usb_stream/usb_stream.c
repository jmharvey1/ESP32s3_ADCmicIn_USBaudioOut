...
//#include "tinyusb/src/common/tusb_private.h"
#include "private_include/usb_stream_descriptor.h"
#include "private_include/usb_host_helpers.h"
#include "usb_stream.h"
#include "private_include/usb_stream_sysview.h"
...
static void _usb_processing_task(void *arg)
{
    ...
    /*The following creates/sets up the USB device using 'usb_phy_init'*/
    usb_dev->port_hdl = _usb_port_init(&_usb_port_callback, (void *)usb_dev->queue_hdl);
    if (usb_dev->port_hdl == NULL)
    {
        sprintf(TxtBuf, "%s:\tUSB Port init failed\n", MyTAG);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    {
        sprintf(TxtBuf, "%s:\tUSB Port init GOOD\n", MyTAG);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    sprintf(TxtBuf, "%s:\tWaiting Connection\n", MyTAG);
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
        xSemaphoreGive(Txt2Dsply_mutx);
    ...
    while (1)
    {
        ...
        if (action_bits == 0)
        {
            ...
            if (++debug_counter % 300 == 0)
            {
                ...
                sprintf(TxtBuf, "%s:\tUSB task running %d\n", MyTAG, debug_counter);
                if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
                {
                    Print2Dsply(TxtBuf);
                    xSemaphoreGive(Txt2Dsply_mutx);
        ...
        if (action_bits & ACTION_DEVICE_CONNECT)
        {
            ...
            // sprintf(TxtBuf, "%s; Setting Port FIFO, %d\n", TAG, usb_dev->fifo_bias);
            // if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
            // {
            //     Print2Dsply(TxtBuf);
            //     xSemaphoreGive(Txt2Dsply_mutx);
            ...
            esp_err_t err = hcd_port_set_fifo_bias(usb_dev->port_hdl, usb_dev->fifo_bias);
            if (err != ESP_OK)
            {
                sprintf(TxtBuf, "%s:\tSet Port FIFO FAILED!!\n", MyTAG);
                if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
                {
                    Print2Dsply(TxtBuf);
                    xSemaphoreGive(Txt2Dsply_mutx);
...
esp_err_t usb_streaming_start()
{
    ...
    if (s_usb_dev.event_group_hdl == NULL)
    {
        sprintf(TxtBuf, "\tevent_group_hdl == NULL\n");
    }
    else
    {
        sprintf(TxtBuf, "\tevent_group_hdl != NULL\n");
    ...
    //  if(s_usb_dev.event_group_hdl != NULL)
    //  {
    //      sprintf(TxtBuf, "Create event group DONE\n");
    //  }
    //  else
    //  {
    //      sprintf(TxtBuf, "Create event group failed\n");
    //  }
    //  if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    //  {
    //      Print2Dsply(TxtBuf);
    ...
    // if(s_usb_dev.queue_hdl != NULL)
    // {
    //     sprintf(TxtBuf, "xQueueCreate USB_EVENT_QUEUE DONE\n");
    // }
    // else
    // {
    //     sprintf(TxtBuf, "xQueueCreate USB_EVENT_QUEUE failed\n");
    // }
    // if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    // {
    //     Print2Dsply(TxtBuf);
    ...
    // if(s_usb_dev.stream_queue_hdl != NULL)
    // {
    //     sprintf(TxtBuf, "xQueueCreate stream_queu DONE\n");
    // }
    // else
    // {
    //     sprintf(TxtBuf, "xQueueCreate stream_queu failed\n");
    // }
    // if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    // {
    //     Print2Dsply(TxtBuf);
    ...
    // if(s_usb_dev.xfer_mutex_hdl != NULL)
    // {
    //     sprintf(TxtBuf, "xfer_mutex_hdl DONE\n");
    // }
    // else
    // {
    //     sprintf(TxtBuf, "Create transfer mutex failed\n");
    // }
    // if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    // {
    //     Print2Dsply(TxtBuf);
    ...
    if (s_usb_dev.ctrl_smp_hdl != NULL)
    {
        sprintf(TxtBuf, "\tctrl_mutex_hdl DONE\n");
    }
    else
    {
        sprintf(TxtBuf, "\tCreate ctrl mutex failed\n");
    }
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
    ...
    sprintf(TxtBuf, "\tPre-alloc ctrl urb succeed, size = %d\n", CTRL_TRANSFER_DATA_MAX_BYTES);
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
        xSemaphoreGive(Txt2Dsply_mutx);
    ...
    if (s_usb_dev.uac_cfg.spk_samples_frequence && s_usb_dev.uac_cfg.spk_bit_resolution)
    {
        ...
        sprintf(TxtBuf, "\tSpeaker instance created\n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    {
        sprintf(TxtBuf, "\tSpeaker Setup SKIPPED\n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    if (s_usb_dev.uac_cfg.mic_samples_frequence && s_usb_dev.uac_cfg.mic_bit_resolution)
    {
        ...
        // ESP_LOGD(TAG, "Speaker instance created");
        sprintf(TxtBuf, "\tMIC instance created\n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    if (s_usb_dev.uvc_cfg.frame_width && s_usb_dev.uvc_cfg.frame_height)
    {
        ...
        sprintf(TxtBuf, "\t%s: Camera instance created\n", TAG);
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    {
        sprintf(TxtBuf, "\tCamera Setup SKIPPED\n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    if (s_usb_dev.enabled[STREAM_UAC_MIC] == true || s_usb_dev.enabled[STREAM_UAC_SPK] == true || s_usb_dev.enabled[STREAM_UVC] == true)
    {
        sprintf(TxtBuf, "\tuac/uvc streaming FULLY configured\n");
    }
    else
    {
        sprintf(TxtBuf, "\tuac/uvc streaming not configured\n");
    }
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
    ...
    if (usbh_taskh != NULL)
    {
        sprintf(TxtBuf, "\t_usb_processing_task UP & RUNNING\n");
    }
    else
    {
        sprintf(TxtBuf, "\tCreate usb processing task failed\n");
    }
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
    ...
    xTaskCreatePinnedToCore(_usb_stream_handle_task, USB_STREAM_NAME, USB_STREAM_STACK_SIZE, NULL,
                            USB_STREAM_PRIORITY, &s_usb_dev.stream_task_hdl, USB_STREAM_CORE); // USB_STREAM_CORE
    // if(s_usb_dev.stream_task_hdl != NULL)
    // {
    //     sprintf(TxtBuf, "\tusb_stream_proc UP & RUNNING\n");
    // }
    // else
    // {
    //     sprintf(TxtBuf, "\tCreate usb_stream_handle_task task failed\n");
    ...
    // sprintf(TxtBuf, "\ts_usb_dev.event_group_hdl %d\n", (int)s_usb_dev.event_group_hdl);
    //  sprintf(TxtBuf, "\ts_usb_dev.event_group_hdl\n");
    //  if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    //  {
    //      Print2Dsply(TxtBuf);
    //      xSemaphoreGive(Txt2Dsply_mutx);
    ...
    sprintf(TxtBuf, "\t%s: USB Streaming Start Succeed\n", TAG);
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
        xSemaphoreGive(Txt2Dsply_mutx);
...
esp_err_t usb_streaming_state_register(state_callback_t cb, void *user_ptr)
{
    ...
    if (s_usb_dev.event_group_hdl)
    {
        sprintf(TxtBuf, "\tUSB streaming is running, \n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
        }
        sprintf(TxtBuf, "callback need register before start\n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    {
        sprintf(TxtBuf, "\tOverwrite registered callback\n");
        if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
        {
            Print2Dsply(TxtBuf);
            xSemaphoreGive(Txt2Dsply_mutx);
    ...
    sprintf(TxtBuf, "\tUSB streaming callback register succeed\n");
    if (pdTRUE == xSemaphoreTake(Txt2Dsply_mutx, pdMS_TO_TICKS(500)))
    {
        Print2Dsply(TxtBuf);
        xSemaphoreGive(Txt2Dsply_mutx);
