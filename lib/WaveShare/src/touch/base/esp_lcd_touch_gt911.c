...
/* this gets fired by lvgl timer automatically to read gt911 xy touch status*/
static esp_err_t esp_lcd_touch_gt911_read_data(esp_lcd_touch_handle_t tp)
{
    esp_err_t err;
    uint8_t buf[41];
...
static esp_err_t touch_gt911_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    assert(tp != NULL);
    /*JMH - long story short, the following two fire 'panel_io_i2c_rx_buffer()' found, in espidf's esp_lcd_panel_io_i2c_v2.c */
    ...
    ESP_LOGI(TAG, "\nTouchPad_ID:0x%02x,0x%02x,0x%02x", buf[0], buf[1], buf[2]); // normal val is 0x39, 0x31, 0x31
    ESP_LOGI(TAG, "TouchPad_Config_Version:%d\n", buf[3]); // normal val is 88
